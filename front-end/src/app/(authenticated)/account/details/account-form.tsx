...
export default function AccountForm({ data }: { data: UserName }) {
  ...
  let disabled = false;
  if (data.email.includes('@epklabs.com')) {
    disabled = true;
  }
  return (
    <form action={updateUserID} className="space-y-8">
      ...
      <p className="text-md text-muted-foreground">
        ...
        at any time. If your account is an ExampleSchool account, please contact
        an administrator to update your email address on the account.
      </p>
      <Separator />
