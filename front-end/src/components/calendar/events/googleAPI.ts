...
export default {
  ...
  getAllCalendars: (config: any) =>
    Promise.map(config.calendars, (calendar: calendars) => {
      // get each calendar
      return fetch(
        `https://content.googleapis.com/calendar/v3/calendars/${calendar.url}/events?key=${config.apiKey}`
      )
        .then((res) => res.json())
        .then((res) => {
          const items = res.items;
          const events = removeCancelled(items);
          const oneTimeEvents = oneTime(
            calendar,
            events
          ) as Array<Schema$Event>;
